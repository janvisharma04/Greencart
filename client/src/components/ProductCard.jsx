...
const ProductCard = ({ product }) => {
    ...
    return product && (
        <div onClick={() => { navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0, 0) }} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full">
            <div className="group cursor-pointer flex items-center justify-center px-2 h-36">
                <img
                    className="group-hover:scale-110 transition duration-200 object-contain max-h-full max-w-full"
                    src={product.image[0]}
                    alt={product.name}
                />
            </div>
